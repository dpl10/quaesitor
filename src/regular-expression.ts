...
const u = {
	...
	alpha: '\\u03B1\\u237A\\u1D6C2\\u1D6FC\\u1D736\\u1D770\\u1D7AA',
	beta: '\\u03B2\\u1D5D\\u1D66\\u1D6C3\\u1D6FD\\u1D737\\u1D771\\u1D7AB',
	bracketOpen: '\\u0028\\u005B',
	bracketClose: '\\u0029\\u005D',
	cross: '\\u00D7\\u2A2F\\u2715\\u2A09\\u2716',
	dagger: '\\u2020',
	gamma: '\\u03B3\\u1D67\\u0194\\u0263\\u0264\\u1D6C4\\u1D6FE\\u1D7AC\\u02E0\\u1D5E',
	quoteOpen: '\\u00AB\\u2039\\u201C\\u201F\\u2018\\u201B\\u0022\\u275B\\u275D\\u276E\\uFF02\\u0027',
	quoteClose: '\\u0022\\u0027\\u00BB\\u2019\\u201A\\u201D\\u201E\\u203A\\u275C\\u276F\\u301D\\u301E\\u301F\\uFF02'
};
/* replacer functions */
...
export class RegularExpression {
	...
	private abbrGenus: RegExp = new RegExp('^[' + u.LatinUpperCase + '][' + u.LatinLowerCase +']{0,2}\\.$');
	private abbrSpecies: RegExp = new RegExp('^[' + u.LatinLowerCase +']{1,3}\\.$');
	...
	private cf: RegExp = new RegExp('^cf\.$');
	public closingExtras: RegExp = new RegExp('^[' + u.dagger + u.quoteClose + u.bracketClose + ']+$', 'g');
	public dash: RegExp = XRegExp('[\\p{Dash_Punctuation}]', 'g');
	private end: RegExp = new RegExp('[,;-' + u.dagger + u.quoteClose + u.bracketClose + ']$');
	private fm: RegExp = new RegExp('^f\\.|fm\\.|fo\\.|fma\\.|form|morph|[' + u.gamma + ']$');
	public html: RegExp = new RegExp('<[^/].*?>|</.+?>', 'g');
	public lineEnding: RegExp = new RegExp('\r\n|\r|\n', 'g');
	private namish: RegExp = new RegExp('^[' + u.LatinUpperCase + ']{0,1}[' + u.LatinLowerCase + ']{1,}$');
	public nomen: RegExp = new RegExp('^([' + u.cross + ']{0,1})([' + u.LatinUpperCase + ']{0,1}[' + u.LatinLowerCase + ']{1,})(\\.{0,1})$');
	public nomenNudem: RegExp = new RegExp('(?=(?:(?:[' + u.cross + u.dagger + u.quoteOpen + u.bracketOpen + ']{1,2}| {0,1})((?:[' + u.LatinUpperCase + '][' + u.LatinLowerCase + ']{1,}|[' + u.LatinUpperCase + '][' + u.LatinLowerCase + ']{0,2}\\.)|(?:[' + u.LatinUpperCase + '][' + u.LatinLowerCase + ']{1,}|[' + u.LatinUpperCase + '][' + u.LatinLowerCase + ']{0,2}\\.) [' + u.cross + '] (?:[' + u.LatinUpperCase + '][' + u.LatinLowerCase + ']{1,}|[' + u.LatinUpperCase + '][' + u.LatinLowerCase + ']{0,2}\\.)) (?:cf\\. ){0,1}(?:(?:sect\\. |§|\\()(?:[' + u.LatinUpperCase + '][' + u.LatinLowerCase + ']{1,}|[' + u.LatinUpperCase + '][' + u.LatinLowerCase + ']{0,2}\\.)\\){0,1} ){0,1}[' + u.cross + ']{0,1}((?:sp{1,2}\\.)|[' + u.LatinLowerCase + ']\\.|[' + u.LatinUpperCase + ']{0,1}[' + u.LatinLowerCase + ']{2,}))).', 'g');
	private notho: RegExp = new RegExp('^[' + u.cross + 'x]$');
	public nothogenus: RegExp = new RegExp('^([' + u.cross + 'x])([' + u.LatinUpperCase + '][' + u.LatinLowerCase + ']{1,})(\\.{0,1})$');
	public nothospecies: RegExp = new RegExp('^([' + u.cross + ']|(?:[' + u.cross + 'x](?=[' + u.LatinUpperCase + '])))([' + u.LatinUpperCase + ']{0,1}[' + u.LatinLowerCase + ']{1,})(\\.{0,1})$');
	...
	private subsp: RegExp = new RegExp('^subsp\\.|[' + u.alpha + ']$');
	private var: RegExp = new RegExp('^var\\.|variety|[' + u.beta + ']$');
	private year: RegExp = new RegExp('^[0-9]{4,4}');
	...
	public abbreviatedSpecies(x: any): boolean {
		if(this.string(x) === true){
			if(x == null){
				return(false);
			} else {
	...
	public authorString(x: any): boolean {
		if(this.string(x) === true){
			if(x == null){
				return(false);
			} else {
	...
	public conferatur(x: any): boolean {
		if(this.string(x) === true){
			if(x == null){
				return(false);
			} else {
	...
	public endName(x: any): boolean {
		if(this.string(x) === true){
			if(x == null){
				return(false);
			} else {
				return(this.end.test(x));
			}
		} else {
			return(false);
		}
	}
	public forma(x: any): boolean {
		if(this.string(x) === true){
			if(x == null){
				return(false);
			} else if(this.year.test(x) === false){ /* alpha, beta, gamma regexp bug */
				return(this.fm.test(x));
			} else {
				return(false);
			}
		} else {
	...
	public name(x: any): boolean {
		if(this.string(x) === true){
			if(x == null){
				return(false);
			} else {
				return(this.namish.test(x));
			}
		} else {
			return(false);
		}
	}
	public nGenus(x: any): boolean {
		if(this.string(x) === true){
			if(x == null){
				return(false);
			} else {
	...
	public subspecies(x: any): boolean {
		if(this.string(x) === true){
			if(x == null){
				return(false);
			} else if(this.year.test(x) === false){ /* alpha, beta, gamma regexp bug */
				return(this.subsp.test(x));
			} else {
				return(false);
			}
		} else {
	...
	public variety(x: any): boolean {
		if(this.string(x) === true){
			if(x == null){
				return(false);
			} else if(this.year.test(x) === false){ /* alpha, beta, gamma regexp bug */
				return(this.var.test(x));
			} else {
				return(false);
			}
		} else {
