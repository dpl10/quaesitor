...
for(k in 1:length(p)){
	x <- epi.tests(as.table(matrix(c(sum(a100[(a100[,'program']==p[k]),'TP']),sum(a100[(a100[,'program']==p[k]),'FP']),sum(a100[(a100[,'program']==p[k]),'FN']),sum(a100[(a100[,'program']==p[k]),'TN'])), nrow = 2, byrow = TRUE)))
...
plot(1, 1, type = 'n', xlab = '', ylab = '', yaxt = 'n', xaxt = 'n', xpd = NA, xlim = c(0.1,0.9), ylim = c(0.2,1.0), col = 'black')
text(0.1, 0.98, pos = 4, labels = expression(bold('A')), cex = cM)
axis(1, cex.axis = cA, las = "1", tcl = -0.2, mgp = c(1, 0.2, 0), xpd = NA, col.axis = 'black', col = 'black')
axis(2, cex.axis = cA, las = "1", tcl = -0.2, mgp = c(1.5, 0.5, 0), xpd = NA, col.axis = 'black', col = 'black')
...
rect(A100[,5], A100[,2], A100[,6], A100[,3], col = makeTransparent('#56ae6c', ((log(A100[,7])-min(log(A100[,7])))/max(log(A100[,7]))*512)+64), angle = 0, border = NA)
points(A100[,4], A100[,1], pch = 22, col = 'black', cex = 1.25, bg = 'black')
text(A100[,4], A100[,1], labels = p, cex = 0.5, col = 'white', font = 2)
### S800
S800 <- matrix(data = NA, nrow = length(p), ncol = 8, byrow = FALSE, dimnames = NULL)
for(k in 1:length(p)){
	x <- epi.tests(as.table(matrix(c(sum(s800[(s800[,'program']==p[k]),'TP']),sum(s800[(s800[,'program']==p[k]),'FP']),sum(s800[(s800[,'program']==p[k]),'FN']),sum(s800[(s800[,'program']==p[k]),'TN'])), nrow = 2, byrow = TRUE)))
...
plot(1, 1, type = 'n', xlab = '', ylab = '', yaxt = 'n', xaxt = 'n', xpd = NA, xlim = c(0.5,1.0), ylim = c(0.2,1.0), col = 'black')
text(0.5, 0.98, pos = 4, labels = expression(bold('B')), cex = cM)
axis(1, cex.axis = cA, las = "1", tcl = -0.2, mgp = c(1, 0.2, 0), xpd = NA, col.axis = 'black', col = 'black')
...
for(k in 1:length(p)){
	x <- epi.tests(as.table(matrix(c(sum(copious[(copious[,'program']==p[k]),'TP']),sum(copious[(copious[,'program']==p[k]),'FP']),sum(copious[(copious[,'program']==p[k]),'FN']),sum(copious[(copious[,'program']==p[k]),'TN'])), nrow = 2, byrow = TRUE)))
...
plot(1, 1, type = 'n', xlab = '', ylab = '', yaxt = 'n', xaxt = 'n', xpd = NA, xlim = c(0.05,0.85), ylim = c(0.2,1.0), col = 'black')
text(0.05, 0.98, pos = 4, labels = expression(bold('C')), cex = cM)
axis(1, cex.axis = cA, las = "1", tcl = -0.2, mgp = c(1, 0.2, 0), xpd = NA, col.axis = 'black', col = 'black')
